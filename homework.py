...
load_dotenv()
# По требованиям заказчика в файле должны быть эти переменные
...
# требованием заказчика логировать то, что происходит внутри функций. В частности
...
def get_api_answer(current_timestamp):
    ...
    try:
        # Закрыта ситуация дисконекта и невозможности преобразования в json
...
def parse_status(homework):
    """Извлеки из входящих данных название домашней работы, её статус...
    и возврати текстовую строку для последующей отсылки телеграм ботом.
    """
...
def check_tokens():
    """Проверь доступность переменных окружения, если нет...
    хотя бы одной переменной, то возрати False, иначе - True; логируй
    на уровне critical каких именно переменных не хватает.
    """
    # Правильно было бы вынести это в начало файла, и свести все токены
    # в словарь, а затем брать значения из словаря, но заказчик требует
...
def main():
    """Сделай запрос к API, проверь ответы, при наличии обновления —...
    получи статус работы из обновления и отправь сообщение в Telegram,
    повтори действия спустя RETRY_TIME, логируй работу в log_config.py.
    """
    ...
    while True:
        try:
            ...
            # поскольку заказчик запретил эту проверку в функции
            # check_response
